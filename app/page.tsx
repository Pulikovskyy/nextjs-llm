...
export default function Home() {
    ...
    code({ node, inline, className, children, ...props }: any) {
      ...
      return !inline && match ? (
        <div onClick={copyToClipboard} className="relative group cursor-pointer">
          <SyntaxHighlighter style={prism} language={match[1]} PreTag="div" customStyle={customStyle} {...props}>
            {codeString}
          </SyntaxHighlighter>
  ...
  const memoizedRenderers = useMemo(() => renderers, []);
  ...
  const handleTextareaChange = (e: any) => {
    ...
    textarea.style.height = Math.min(textarea.scrollHeight, window.innerHeight * 0.33) + 'px'; // Set to scrollHeight or 33% of window height, whichever is smaller
      ...
      <div className="relative z-10">
        ...
        <div className="px-4">
          <textarea
            ...
            style={{
              height: 'auto',
              maxHeight: `${window.innerHeight * 0.33}px`,
            }}
            value={prompt}
                ...
                .map((message, index) => (
                  ...
                  >
                    ...
                    <div style={{ display: collapsed[index] ? 'none' : 'block' }}>
                      <ReactMarkdown components={memoizedRenderers}>{message.content}</ReactMarkdown>
                    </div>
